...
public class ChatController {
    ...
    private void onPhotoClick() {
        ...
        MenuItem galleryItem = new MenuItem("🖼 From Gallery");
        galleryItem.setOnAction(e -> handlePhotoFromGallery());
        
        MenuItem cameraItem = new MenuItem("📷 From Camera");
        cameraItem.setOnAction(e -> {
            Stage stage = (Stage) photoButton.getScene().getWindow();
            com.catconnect.util.CameraModal.show(stage, file -> handleCapturedPhoto(file));
        });
        
        menu.getItems().addAll(galleryItem, cameraItem);
        menu.show(photoButton, javafx.geometry.Side.TOP, 0, 0);
    }
    
    private void handlePhotoFromGallery() {
    ...
    private void handleCapturedPhoto(File file) {
        if (file == null) return;
        processImageFile(file);
    }
